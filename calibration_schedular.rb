...
while true do
  ...
  if reading > 0
    ...
    puts "Flow began... average at #{average}"
  else
    puts "Awaiting flow or sensor read incorrectly..."
    next
  end
  if count == 100
    puts "Calibrated after 100 readings... finding systol/diastol cycles..."
  end
  if count >= 100 and count < 300
    ...
    if reading > highest && stdev < 3 && stdev > -3
      highest << reading
      puts "New high of #{highest} found."
    elsif reading < lowest && stdev <3 && stdev >-3
      lowest << reading
  ...
  if count == 300
    ...
    avg_diastol = lowest.reduce(:+) / highest.size
